...
EOF
    ...
    # Merge onboarding json files
    if test -f "/mnt/external_files/configs/onboarding.json"; then
      mv /usr/share/nginx/html/onboarding.json /usr/share/nginx/html/onboarding.json.bak
      /opt/clearml/utilities/merge_onboarding_files.py \
        --input-file /usr/share/nginx/html/onboarding.json.bak \
        --input-file /mnt/external_files/configs/onboarding.json \
        --output-file /usr/share/nginx/html/onboarding.json
    fi

    # Merge image folders
    if test -d "/mnt/external_files/configs/tips-images"; then
      cp -af "/mnt/external_files/configs/tips-images" /usr/share/nginx/html
    fi
...
elif [[ ${SERVER_TYPE} == "db-prepopulate-export" ]]; then
    ENVIRONMENT=$2
    PROJECT=$3
    UUID=$4
    cd /opt/clearml/apiserver/
    python3 /opt/clearml/apiserver/data_tool.py export \
      --output /opt/clearml/db-pre-population-export/${UUID}.zip \
      --upload allegro-data-transfer/trains-demo-pre-populate/${ENVIRONMENT}/ \
      --projects $PROJECT

elif [[ ${SERVER_TYPE} == "db-prepopulate-import" ]]; then
    ENVIRONMENT=$2
    cd /opt/clearml/apiserver/
    PYTHONPATH=/opt/clearml/ python3 /opt/clearml/apiserver/data_tool.py import \
      /opt/clearml/db-pre-populate \
      --download allegro-data-transfer/trains-demo-pre-populate/${ENVIRONMENT}/ \
      --user __allegroai__ \
      --user-name "Allegro.ai"
